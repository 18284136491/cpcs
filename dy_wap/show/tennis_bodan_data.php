...
$sql		=	"select id from tennis_Match where Match_Type=10 and Match_Bd21>0 and  Match_CoverDate>now() ".$sqlwhere.' order by Match_CoverDate,match_name';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name tennis_Match where Match_Type=10 and Match_Bd21>0 and  Match_CoverDate>now() group by match_name";
	$query	=	$mysqlis->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
