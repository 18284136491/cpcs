...
$sql		=	"select id from baseball_match WHERE Match_Type=1 and Match_CoverDate>now() and Match_BzM>0 and Match_Date='".date("m-d")."' ".$sqlwhere.' order by Match_CoverDate,match_name';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from baseball_match WHERE Match_Type=1 and Match_CoverDate>now() and Match_BzM>0 and Match_Date='".date("m-d")."' group by match_name";
	$query	=	$mysqlis->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
