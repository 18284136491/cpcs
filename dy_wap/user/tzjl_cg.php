...
$sql		=	"select g.gid,date_add(g.bet_time, interval 25 hour) as bet_time,g.cg_count,g.bet_money,g.win,g.bet_win from k_bet_cg_group g where g.status in(0,2) and uid=$uid order by g.bet_time desc";
  ...
  <div class="container">
	    ...
	    <div role="tabpanel">
	    	...
	    	<!-- Nav tabs -->
			  <ul class="nav nav-tabs nav-tile" role="tablist">
			    ...
			    <li class="active"><a href="tzjl_cg.php">体育串关</a></li>
			    <li><a href="tzjl_cp.php">彩票游戏</a></li>
			  </ul>
			<!-- Tab panes -->
    ...
    <tr bgcolor="<?=$bgcolor?>" align="center" onMouseOver="this.style.backgroundColor='#FFFFCC'" onMouseOut="this.style.backgroundColor='<?=$bgcolor?>'" >
	  ...
	  <td><span style="color:#46AF98">下注:</span><br><?=$rows["bet_money"]?><br><span style="color:#0DC4FF">可赢:</span><br><?php
		  echo double_format($rows["bet_win"]+$rows["fs"]);
	?></td>
    </tr>
<?php
	}
...
?>
    ...
    <div class="panel-footer">  	
  	<ul class="pagination"><?=$page->get_htmlPage($_SERVER["REQUEST_URI"]);?> <li><a href="javascript:;">总投注金额：<span style="color:#FF0000"><?=double_format(@$bet_money)?></span>，可赢：<span style="color:#FF0000"><?=double_format(@$ky)?></span></a></li></ul>
  </div>
			    	</div>
			    </div>
			   </div>
