...
$sql		=	"select bid from k_bet where `status`>0 and uid=$uid and match_coverdate>='$stime' and match_coverdate<'$etime' and (ball_sort!='重庆时时彩' AND ball_sort!='广东快乐十分' AND ball_sort!='北京PK10' AND ball_sort!='福彩3D' AND ball_sort!='排列3' AND ball_sort!='上海时时乐') ".$fsql." order by bid desc";
}else
{
	$sql		=	"select bid from k_bet where `status`>0 and uid=$uid and match_coverdate>='$stime' and match_coverdate<'$etime' and (ball_sort='重庆时时彩' or ball_sort='广东快乐十分' or ball_sort='北京PK10' or ball_sort='福彩3D' or ball_sort='排列3' or ball_sort='上海时时乐') ".$fsql." order by bid desc";
}
    ...
    <tr bgcolor="<?=$bgcolor?>" align="center" onMouseOver="this.style.backgroundColor='#FFFFCC'" onMouseOut="this.style.backgroundColor='<?=$bgcolor?>'" style="height:60px;" >
	  ...
	  <span style="color:#005481"><b><?=$rows["match_name"]?></b></span>
       <?php
		...
		$m_count=count($m);
		preg_match('[\((.*)\)]', $m[$m_count-1], $matches);
	   	if($rows["ball_sort"]=='重庆时时彩' || $rows["ball_sort"]=='福彩3D' || $rows["ball_sort"]=='排列3' || $rows["ball_sort"]=='上海时时乐' || $rows["ball_sort"]=='广东快乐十分' || $rows["ball_sort"]=='北京PK10'){
	  ...
	  <td><?php
		...
		switch ($rows["status"]){
			case "1":
				...
				echo '<span style="color:#FF0000;">'.double_format($jine).'</span>';
				break;
			case "2":
				$jine=$rows["win"]+$rows["fs"];
				echo '<span style="color:#000000;">'.double_format($jine).'</span>';
				break;
			case "4":
				$jine=$rows["win"]+$rows["fs"];
				echo '<span style="color:#FF0000;">'.double_format($jine).'</span>';
