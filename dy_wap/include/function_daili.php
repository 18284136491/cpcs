		...
		if($max_ratio) {
			if($is_zongdaili) {
				...
				if($rows) {
					...
					$result["cs_fc_ratio"] = $rows['cs_fc_ratio'];
					$result["cs_lt_ratio"] = $rows['cs_lt_ratio'];
				} else {
					...
					$result["cs_fc_ratio"] = $default_config['cs_zd_fc_ratio'];
					$result["cs_lt_ratio"] = $default_config['cs_zd_lt_ratio'];
				}
			}
		} else {
			...
			if($rows) {
				if($is_zongdaili!=1 || $result["daili_mode"]==0) {
					$result["zc_zd_ty_ratio"] = $rows['zc_zd_ty_ratio'];
					$result["zc_dl_ty_ratio"] = $rows['zc_dl_ty_ratio'];
					$result["zc_yx_ratio"] = $rows['zc_yx_ratio'];
					$result["zc_fc_ratio"] = $rows['zc_fc_ratio'];
				...
				if($is_zongdaili!=1 || $result["daili_mode"]==1) {
					$result["cs_ratio"] = $rows['cs_ratio'];
					$result["cs_fc_ratio"] = $rows['cs_fc_ratio'];
					$result["cs_lt_ratio"] = $rows['cs_lt_ratio'];
				}
				...
				if($is_zongdaili!=1) {
					...
					$result["cs_fc_ratio"] = $default_config['cs_dl_fc_ratio'];
					$result["cs_lt_ratio"] = $default_config['cs_dl_lt_ratio'];
				}
			}
		}
	} else {
...
function getDefaultRatio() {
	...
	if($default_rows) {
		...
		$default_config["cs_zd_ratio"]    = $default_rows['cs_zd_ratio'];
		$default_config["cs_dl_ratio"]    = $default_rows['cs_dl_ratio'];
		$default_config["cs_zd_fc_ratio"] = $default_rows['cs_zd_fc_ratio'];
		$default_config["cs_dl_fc_ratio"] = $default_rows['cs_dl_fc_ratio'];
		$default_config["cs_zd_lt_ratio"] = $default_rows['cs_zd_lt_ratio'];
		$default_config["cs_dl_lt_ratio"] = $default_rows['cs_dl_lt_ratio'];
	}
...
function calculateGainsAndLosses($uid, $user, $month, $daili=false) {
	...
	$sql	=	"select bet_money,win,`status`,match_coverdate,fs,bet_point,point_column from k_bet where `status`>0 and uid in ($uid) and match_coverdate like('".$month."%') and (ball_sort!='重庆时时彩' AND ball_sort!='广东快乐十分' AND ball_sort!='北京PK10' AND ball_sort!='福彩3D' AND ball_sort!='排列3' AND ball_sort!='上海时时乐') order by match_coverdate asc"; //单式
	//echo $sql;exit;
	$query	=	$mysqli->query($sql);
	while($rows	=	$query->fetch_array()){
		$ts		=	$rows['fs'];
		if($rows['status']=="1" || $rows['status']=="4"){ //赢和赢一半都算赢
			...
			if($rows['status']=="1") {
			    ...
			    if($bet_point>1) {
				    ...
				    $gainsAndLosses["ty_yx"] += $rows['bet_money'] * $bet_point;
	...
	$sql	=	"select bet_money,win,`status`,match_coverdate,fs from k_bet where `status`>0 and uid in ($uid) and match_coverdate like('".$month."%') and (ball_sort='重庆时时彩' or ball_sort='广东快乐十分' or ball_sort='北京PK10' or ball_sort='福彩3D' or ball_sort='排列3' or ball_sort='上海时时乐') order by match_coverdate asc"; //单式
	//echo $sql;exit;
	$query	=	$mysqli->query($sql);
	while($rows	=	$query->fetch_array()){
		$ts		=	$rows['fs'];
...
function calculateDailiTotal($uid, $user, $month) {
	...
	$daili_total["detail"]["lotto"] = 0 - $daili_total["yk"]["lotto"];//$daili_total["yk"]["lotto"];//
