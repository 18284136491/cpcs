...
if($_POST["point_column"]=='Match_DFzDpl' || $_POST["point_column"]=='Match_DFzXpl'){//exit($rows["Match_DFzDX"]);
	$match_dxgg		=	$rows["Match_DFzDX"];
}elseif($_POST["point_column"]=='Match_DFkDpl' || $_POST["point_column"]=='Match_DFkXpl'){
	$match_dxgg		=	$rows["Match_DFkDX"];
}else{
...
<input type="hidden" name="match_dxgg[]" value="<?=$match_dxgg?>"  />
