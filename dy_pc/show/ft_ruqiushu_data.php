...
$sql = "select id from bet_match where Match_Type=1 and Match_IsShowt=1 and Match_Total01Pl>0 AND Match_CoverDate>now()" . $sqlwhere . ' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if ($i == 1) { //未查找到记录
    ...
    $sql = "select match_name from bet_match where Match_Type=1 and Match_IsShowt=1 and Match_Total01Pl>0 AND Match_CoverDate>now() group by match_name";
    $query = $mysqlis->query($sql);
    $i = 0;
    $lsm = '';
    while ($row = $query->fetch_array()) {
