	...
	$.getJSON("ftz_ruqiushu_data.php?leaguename="+league+"&CurrPage="+thispage+"&callback=?",function(json){
		...
		if(pagecount == "error1"){
			...
			for(var i=0; i<dbs.length; i++){
				if(dbs[i]["Match_BzM"] !="0" || dbs[i]["Match_Total01Pl"] !="0" || dbs[i]["Match_Total23Pl"] !="0" || dbs[i]["Match_Total46Pl"] !="0" || dbs[i]["Match_Total7upPl"] !="0"){
			...
			htmls+="<div onmouseover=\"this.className='d_over'\" onmouseout=\"this.className='d_out'\">";	
				...
				htmls +="<div class='zhudui'>";
					...
					htmls +="<div class='hui_x zu_ruqiu_zu'>"+((dbs[i]["Match_BzM"] !=null && dbs[i]["Match_BzM"] !="0")?"<a href=\"javascript:void(0)\" title=\""+dbs[i]["Match_Master"]+"\" onclick=\"setbet('足球早餐','标准盘-"+ dbs[i]["Match_Master"] +"-独赢','" + dbs[i]["Match_ID"] + "','Match_BzM','0','0','"+dbs[i]["Match_Master"]+"');\" style='"+(dbs[i]["Match_BzM"]!=data[i]["Match_BzM"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_BzM"],2)+"</a>":"")+"</div>";
				...
				htmls +="<div class='kedui'>";
					...
					htmls +=" <div class='hui_x2 zu_ruqiu_zu'>"+(dbs[i]["Match_BzG"] !=null && dbs[i]["Match_BzG"] !="0"?"<a href=\"javascript:void(0)\" title=\""+dbs[i]["Match_Guest"]+"\" onclick=\"setbet('足球早餐','标准盘-"+ dbs[i]["Match_Guest"] +"-独赢','" + dbs[i]["Match_ID"] + "','Match_BzG','0','0','"+dbs[i]["Match_Guest"]+"');\" style='"+(dbs[i]["Match_BzG"]!=data[i]["Match_BzG"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_BzG"],2)+"</a>":"")+"</div>";
					htmls +="<div class='hui_x zu_ruqiu_3'>"+((dbs[i]["Match_Total01Pl"] !=null && dbs[i]["Match_Total01Pl"] !="0")?"<a href=\"javascript:void(0)\" title=\"0~1\" onclick=\"setbet('足球早餐','入球数-0~1','" + dbs[i]["Match_ID"] + "','Match_Total01Pl','0','0','0~1');\" style='"+(dbs[i]["Match_Total01Pl"]!=data[i]["Match_Total01Pl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_Total01Pl"],2)+"</a>":"")+"</div>";
					htmls +=" <div class='hui_x zu_ruqiu_3'>"+((dbs[i]["Match_Total23Pl"] !=null && dbs[i]["Match_Total23Pl"] !="0")?"<a href=\"javascript:void(0)\" title=\"2~3\" onclick=\"setbet('足球早餐','入球数-2~3','" + dbs[i]["Match_ID"] + "','Match_Total23Pl','0','0','2~3');\" style='"+(dbs[i]["Match_Total23Pl"]!=data[i]["Match_Total23Pl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_Total23Pl"],2)+"</a>":"")+"</div>";
					...
					htmls +=" <div class='hui_z zu_ruqiu_3'>"+((dbs[i]["Match_Total7upPl"] !=null && dbs[i]["Match_Total7upPl"] !="0")?"<a href=\"javascript:void(0)\" title=\"7以上\" onclick=\"setbet('足球早餐','入球数-7UP','" + dbs[i]["Match_ID"] + "','Match_Total7upPl','0','0','7UP');\" style='"+(dbs[i]["Match_Total7upPl"]!=data[i]["Match_Total7upPl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_Total7upPl"],2)+"</a>":"")+"</div>";
				htmls +="</div>";
				htmls +="<div class='heju'>";
					htmls +="  <div class='hui_x_hj heju_zu_ruqiu_2'>和局</div>";
					htmls +=" <div class='hui_x_hj zu_ruqiu_zu'>"+((dbs[i]["Match_BzH"] !=null && dbs[i]["Match_BzH"] !="0")?"<a href=\"javascript:void(0)\" title=\"和局\" onclick=\"setbet('足球早餐','标准盘-和局','" + dbs[i]["Match_ID"] + "','Match_BzH','0','0','和局');\" style='"+(dbs[i]["Match_BzH"]!=data[i]["Match_BzH"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_BzH"],2)+"</a>":"")+"</div>";
					htmls +=" <div class='hui_x zu_ruqiu_zu2'></div>";
					htmls +=" <div class='hui_x zu_ruqiu_zu2'></div>";
					htmls +=" <div class='hui_x zu_ruqiu_zu2'></div>";
					htmls +=" <div class='hui_z zu_ruqiu_zu2'></div>";
...
$(document).ready(function(){
	$("#xzls").click(function(){ //选择联赛
		JqueryDialog.Open('足球早餐入球数', 'dialog.php?lsm='+window_lsm, 600, window_hight);
	});
});