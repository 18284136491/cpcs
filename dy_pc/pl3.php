...
$sql = "select id,class1,class2,class3,odds,modds,locked from lottery_odds where class1='pl3' and class2='和值' order by ID asc";
...
<head>
	...
	function addzhbet(){
		...
		closeadds()
		<? if($stype==1){?>
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=1&bw="+bwhm+"&sw="+swhm+"&gw="+gwhm+"",
		<? }?>
		<? if($stype==2){?>
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=2&sw="+swhm+"",
		<? }?>
		<? if($stype==3){?>
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=3&bw="+bwhm+"&sw="+swhm+"",
		<? }?>
		<? if($stype==4){?>
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=4&bw="+bwhm+"&sw="+swhm+"&gw="+gwhm+"",
		<? }?>
		<? if($stype==5){?>
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=5&bw="+bwhm+"&sw="+swhm+"&gw="+gwhm+"",
		<? }?>
		<? if($stype==6){?>
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=6&bw="+bwhm+"",
		<? }?>
		{
			id:'opens',
			lock:true,
	...
	function hz_add(id){
		...
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=7&content="+id+"",
		{
			id:'opens',
			fixed: true,
			lock:true,
	...
	function dsdx_add(class3,id){
		...
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=8&class3="+class3+"&content="+id+"",
		{
			id:'opens',
			fixed: true,
			lock:true,
	...
	function kd_add(id){
		...
		art.dialog.open("buy_lottery/pl3.php?uid=<?=$uid?>&stype=6&content="+id+"",
		{
			id:'opens',
			fixed: true,
			lock:true,
...
<div class="left_lottery_bg1"><? if($tcou>0){?>体彩排列3第 <font color="#00FF00"><?=$trow['qihao']?></font> 期<? }else{?>期数未开盘<? }?></div>
...
<?
$sqlk = "select * from lottery_k_pl3 where ok=1 order by qihao desc, id desc limit 10";
