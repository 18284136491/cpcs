...
function action(){
    $.getJSON("systopDao.php?callback=?",function(json){
			...
			if(sum > 0){
				var html = "您有：";
				//先判断是否有有提示音
				if(json.tknum > 0 && (json.cknum == '' || json.cknum == 0) && (json.hknum == '' || json.hknum == 0)){
					$("#hk_mp3").html(""); //先清空，再添加提示声音
					document.getElementById('hk_mp3').innerHTML= "<embed src='/date/tk.mp3' width='0' height='0'></embed>";
				}
				if((json.tknum == '' || json.tknum == 0) && (json.cknum > 0 || json.hknum > 0)){
					$("#hk_mp3").html(""); //先清空，再添加提示声音
					document.getElementById('hk_mp3').innerHTML= "<embed src='/date/hk.mp3' width='0' height='0'></embed>";
				}
				if(json.tknum > 0 && (json.cknum > 0 || json.hknum > 0)){
					$("#hk_mp3").html(""); //先清空，再添加提示声音
					document.getElementById('hk_mp3').innerHTML= "<embed src='/date/tk.mp3' width='0' height='0'></embed>";
				}
				if(json.tknum > 0){
				...
				if(json.cknum > 0){
					html += "<span class=\"num\">"+json.cknum+"</span> 条<strong>存款</strong> ";
					//$("#hk_mp3").html(""); //先清空，再添加提示声音
					//document.getElementById('hk_mp3').innerHTML= "<embed src='/date/tk.mp3' width='0' height='0'></embed>";
				}
				if(json.hknum > 0){
