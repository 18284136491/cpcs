...
if($username == ''){
	if($type == "Y"){ //查询所有过存款/取款/交易的会员
		$sql	=	"select uid from k_bet where match_coverdate>='$date_start' and match_coverdate<='$date_end' and lose_ok=1 and `status`>0 and uid not in('$hl_uid')";
		$query	=	$mysqli->query($sql);
		while($rows = $query->fetch_array()){
			$arr_uid[$rows['uid']]	=	$rows['uid'];
		}
		
		$sql	=	"select uid from k_bet_cg_group where `status` in(1,3) and match_coverdate>='$date_start' and match_coverdate<='$date_end' and uid not in('$hl_uid')";
		$query	=	$mysqli->query($sql);
		while($rows = $query->fetch_array()){
			$arr_uid[$rows['uid']]	=	$rows['uid'];
		}
	
		$sql	=	"select uid from k_money where `status`=1 and type=1 and m_make_time>='$date_start1' and m_make_time<='$date_end1' and uid not in('$hl_uid')";
		$query	=	$mysqli->query($sql);
		while($rows = $query->fetch_array()){
			$arr_uid[$rows['uid']]	=	$rows['uid'];
		}
		
		$sql	=	"select uid from huikuan where `status`=1 and adddate>='$date_start1' and adddate<='$date_end1' and uid not in('$hl_uid')";
		$query	=	$mysqli->query($sql);
		while($rows = $query->fetch_array()){
			$arr_uid[$rows['uid']]	=	$rows['uid'];
		}
		...
		$sql	=	"select uid from k_bet_cg_group where `status` in(0,2) and match_coverdate>='$date_start' and match_coverdate<='$date_end' and uid not in('$hl_uid')";
		$query	=	$mysqli->query($sql);
		while($rows = $query->fetch_array()){
			$arr_uid[$rows['uid']]	=	$rows['uid'];
		}
	...
	$sql 		=	"select uid from k_user where username in ($username) and uid not in('$hl_uid')";
	$query		=	$mysqli->query($sql);
	while($rows =	$query->fetch_array()){
		$arr_uid[$rows['uid']]	=	$rows['uid'];
	}
