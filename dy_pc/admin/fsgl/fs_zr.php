...
if($addfs==1){
	...
	$fs_order = "ZR".date("ymdHis").rand(10,99);
	
	if($fs_num==0){
		message("没有记录，无需返水！","fs_zr.php");
	}
	...
	for($i=0;$i<$fs_num;$i++){
		...
		if($fs>0){
			...
			$sql .=" value('$uid','$username','$fs_btime','$fs_etime','$fs_fs','真人','$num','$bet_money','$yx_bet_money','$yingli','$fs_name','$fs_rate','$fs','$fs_order')";
			$mysqli->query($sql);
			...
			$assets = $row['money'];
			$balance = $assets+$fs;
			$m_order = $username."_zrfs_".$fs_id;
			
			$sql = "insert into k_money (uid,m_value,m_order,status,about,type,sxf,assets,balance)";
			$sql .=" value('$uid','$fs','$m_order','2','真人自动返水[管理员结算]','5','0','$assets','$balance')";
			$mysqli->query($sql);
	...
	message("返水成功！返水批次号是：$fs_order","fs_zr.php");
}
?>
...
<div id="pageMain">
	<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="font12" style="border:1px solid #798EB9;">
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				&nbsp;返水彩种
				<input name="fscz" type="checkbox" checked="checked" value="zr" disabled="disabled" />真人
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;line-height:20px;" bgcolor="#798EB9">
		...
		$sql = "select B.uid,B.username,A.playerName,count(A.id) as num,sum(A.betAmount) as bet_money,sum(A.validBetAmount) as yx_bet_money,sum(-A.netAmount) as yingli from agbetdetail A left join k_user B on A.playerName=B.ag_zr_username where A.betTime>='$q_btime' and A.betTime<='$q_etime' $sqlwhere group by A.playerName";
		$query = $mysqli->query($sql);
		...
		<tr align="center" style="background:#ffffff;color:#ff0000;">
			<td colspan="9"><input name="account" type="submit" id="account" value="返水到会员账户"/></td>
		</tr>
	</table>
	</form>
	<?php } ?>
