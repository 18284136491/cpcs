...
if($addfs==1){
	...
	$fs_order = "CP".date("ymdHis").rand(10,99);
	
	if($fs_num==0){
		message("没有记录，无需返水！","fs_cp.php");
	}
	...
	for($i=0;$i<$fs_num;$i++){
		...
		if($fs>0){
			...
			$sql .=" value('$uid','$username','$fs_btime','$fs_etime','$fs_fs','彩票','$num','$bet_money','$yx_bet_money','$yingli','$fs_name','$fs_rate','$fs','$fs_order')";
			$mysqli->query($sql);
			...
			$assets = $row['money'];
			$balance = $assets+$fs;
			$m_order = $username."_cpfs_".$fs_id;
			
			$sql = "insert into k_money (uid,m_value,m_order,status,about,type,sxf,assets,balance)";
			$sql .=" value('$uid','$fs','$m_order','2','彩票自动返水[管理员结算]','5','0','$assets','$balance')";
			$mysqli->query($sql);
	...
	message("返水成功！返水批次号是：$fs_order","fs_cp.php");
}
?>
...
<div id="pageMain">
	<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0" class="font12" style="border:1px solid #798EB9;">
		...
		<tr bgcolor="#FFFFFF">
			<td align="left">
				&nbsp;返水彩种
				<input name="fscz" type="checkbox" checked="checked" value="cp" disabled="disabled" />彩票
	...
	<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;line-height:20px;" bgcolor="#798EB9">
		...
		$sql	=	"select uid,username,sum(num) as num,sum(bet_money) as bet_money,sum(yx_bet_money) as yx_bet_money,sum(bet_money-win) as yingli from (";
		$sql	.=	"select uid,username,1 as num,money as bet_money,if(win<>0,money,0) as yx_bet_money,case when win<0 then 0 when win=0 then money else win end as win from c_bet where money>0 and js=1 and addtime>='$q_btime' and addtime<='$q_etime' ".$sqlwhere;
		$sql	.=	" union all ";
		$sql	.=	"select uid,username,1 as num,money as bet_money,if(win<>0,money,0) as yx_bet_money,case when win<0 then 0 when win=0 then money else win end as win from c_bet where money>0 and js=1 and addtime>='$q_btime' and addtime<='$q_etime' ".$sqlwhere;
		$sql	.=	" union all ";
		$sql	.=	"select u.uid,k.username,1 as num,k.money as bet_money,if(k.win<>0,k.money,0) as yx_bet_money,k.win+k.money as win from lottery_data k left join k_user u on k.username=u.username where k.money>0 and k.bet_ok=1 and k.bet_time>='$q_btime' and k.bet_time<='$q_etime' ".$sqlwhere;
