...
if($_GET["action"]=="add" && $id==0){ 
	...
	$sql		=	"insert into c_auto_$gameId(qishu,datetime,ball_1,ball_2,ball_3,ball_4) values (".$qishu.",'".$datetime."',".$ball_1.",".$ball_2.",".$ball_3.",".$ball_4.")";
	$mysqli->query($sql);
	...
	$ball_3		=	$_POST["ball_3"];
	$ball_4		=	$ball_1+$ball_2+$ball_3;
	$sql		=	"update c_auto_$gameId set qishu=".$qishu.",datetime='".$datetime."',ball_1=".$ball_1.",ball_2=".$ball_2.",ball_3=".$ball_3.",ball_4=".$ball_4." where id=".$id."";
	$mysqli->query($sql);
}
...
?>
  ...
  <tr>
    ...
    <td  align="left" bgcolor="#FFFFFF"><?=$gameName?>【<a href="Uptime_12.php?type=<?=$gameId?>" style="color:#ff0000;">点击进入盘口管理</a>】</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:5px;" bgcolor="#798EB9">   
            <tr style="background-color:#3C4D82; color:#FFF">
              ...
              <td align="center"><strong>特码</strong></td>
        <td align="center"><strong>大小单双</strong></td>
        <td align="center"><strong>波色</strong></td>
        <td height="25" align="center"><strong>极值</strong></td>
...
if($id){
	...
	while($rows = $query->fetch_array()){
		...
		$hm[]		= $rows['ball_4'];
		if($rows['ok']==1){
			$ok = '<a href="../cj/lottery/js_5.php?qi='.$rows['qishu'].'&t=1"><font color="#FF0000">已结算</font></a>';
		}else{
			$ok = '<a href="../cj/lottery/js_5.php?qi='.$rows['qishu'].'&t=1"><font color="#0000FF">未结算</font></a>';
		}
?>
      <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
        ...
        <td><?=xy28_Auto($hm,4)?></td>
        <td><?=xy28_Auto($hm,6)?></td>
        <td><?=xy28_Auto($hm,5)?></td>
        <td><?=$ok?></td>
