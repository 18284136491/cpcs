  ...
  <table width="100%" height="100%" border="0" cellspacing="0" cellpadding="5">
    <tr>
	  ...
	  <?php include_once("Menu_Auto.php"); ?>
        <form name="form1" onSubmit="return check_submit();" method="post" action="?id=<?=$id?>&type=<?=$gameId?>&action=<?=$id>0 ? 'edit' : 'add'?>&page=<?=$_REQUEST['page']?>">
<?php
...
?>
  ...
  <tr>
    ...
    <td  align="left" bgcolor="#FFFFFF">北京快乐8【<a href="Uptime_2.php?type=<?=$gameId?>" style="color:#ff0000;">点击进入盘口管理</a>】</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	20球<input type="text" name="ball_200" id="ball_20"size="5" value="<?=$rs['ball_20']?>"/>&nbsp;</td>
  </tr>
  <tr>
...
<?php
$sql		=	"select id from c_auto_$gameId order by qishu desc";
$query		=	$mysqli->query($sql);
...
if($id){
	...
	while($rows = $query->fetch_array()){
		$color = "#FFFFFF";
	 	...
	 	$out	 = "#ffffff";
		if($rows['ok']==1){
			$ok = '<a href="../cj/lottery/js_1.php?qi='.$rows['qishu'].'&t=1"><font color="#FF0000">已结算</font></a>';
		}else{
			$ok = '<a href="../cj/lottery/js_1.php?qi='.$rows['qishu'].'&t=1"><font color="#0000FF">未结算</font></a>';
		}
?>
      <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
        ...
        <td><?=$ok?></td>
        <td><a href="?id=<?=$rows["id"]?>&type=<?=$gameId?>&page=<?=$_REQUEST['page']?>">编辑</a></td>
        </tr>
<?php
	}
}
