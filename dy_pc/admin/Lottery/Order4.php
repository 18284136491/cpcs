...
{
	$sql="select * from c_bet where type='北京赛车PK拾' and id=$id";
	...
	if($rs)
	{
		...
		//删除订单
		$sql="delete from c_bet where type='北京赛车PK拾' and id=$id";
		$mysqli->query($sql);
		...
		$message="撤销[".$kusername."]北京赛车PK拾注单[".$id."],[注单金额:".$money.",可赢金额:".$win.",结算状态:".$js."],退回金额:".$remoney;
...
<?php
      ...
      $sql	=	"select id from c_bet where money>0 and type='北京赛车PK拾' ";
