...
//获取开奖号码
$sql		= "select * from c_auto_13 where ok=0";
...
$hm[]		= $rs['ball_5'];
//根据期数读取未结算的注单
$sql1		= "select * from c_bet where type='加拿大28' and js=0 and qishu=".$qi." order by addtime asc";
...
while($rows = $query1->fetch_array()){
	...
	if($rows['mingxi_1']=='混合玩法'){
		...
		if(($rows['mingxi_2']=='小' || $rows['mingxi_2']=='单') && ($rs['ball_4']==13)){
			$sql2		= "select SUM(money) as num from c_bet where type='加拿大28' and mingxi_2='".$rows['mingxi_2']."' and qishu=".$qi." and uid=".$rows['uid'];
...
$msql="update c_auto_13 set ok=1 where qishu=".$qi."";
