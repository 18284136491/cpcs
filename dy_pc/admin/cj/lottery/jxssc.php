...
for($i=1;$i<=84;$i++){
	//echo $i."===";
	$sqls="select * from c_opentime_7 where qishu='$i'";
	//echo $sqls;
	...
	if($rs['qishu']){
		...
		$strSqls="UPDATE `c_opentime_7` SET `kaipan`='$actiontime',`fengpan`='$fengpan',`kaijiang`='$kaijiang' WHERE (`qishu`='".$rs['qishu']."')";
		//echo $strSqls."<br>";
...
$curl->set_referrer("http://www.1680180.com/lottery/10021.html");
...
$html_data = $curl->fetch_url("http://www.1680180.com/Open/CurrentOpen?code=10021&_=0.3424344145450467");
$a=array('(',')');
$b=array('[',']');
  ...
  <tr> 
    <td align="left">
      <input type=button name=button value="刷新" onClick="window.location.reload()">
        天津时时彩<br>采集到<?=$m?>期(<?=$qishu?>期<?="$ball_1,$ball_2,$ball_3,$ball_4,$ball_5"?>):
