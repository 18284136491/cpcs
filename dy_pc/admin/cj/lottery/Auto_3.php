...
$curl->set_referrer("http://www.1680180.com/lottery/1008.html");
...
$html_data = $curl->fetch_url("http://www.1680180.com/Open/CurrentOpen?code=1008&_=0.22965576034578294");
$a=array('(',')');
$b=array('[',']');
...
if($arr['l_r']){
	...
	if(strlen($qishu)>0){
		...
		if($tcou==0){
			$sql 	= "select kaijiang from `c_opentime_3` where qishu='".intval($qishunum)."'";
			$query 	= $mysqli->query($sql);
			$rs		= $query->fetch_array();
			$time   = "$Y-$M-$D ".$rs['kaijiang'];
			$sql	=	"insert into c_auto_3(qishu,datetime,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values ('$qishu','$time','$ball_1','$ball_2','$ball_3','$ball_4','$ball_5','$ball_6','$ball_7','$ball_8')";
		//echo $sql.'<br />';
			$mysqli->query($sql);
			$m=$m+1;
		}
...
<iframe src="js_3.php?qi=<?=$qishu?>" frameborder="0" scrolling="no" height="0" width="0"></iframe>
</body>
</html>