...
for($i=1;$i<=179;$i++){
	echo $i."===";
	$sqls="select * from c_opentime_4 where qishu='$i'";
	echo $sqls;
	...
	if($rs['qishu']){
		...
		$strSqls="UPDATE `c_opentime_4` SET `kaipan`='$actiontime',`fengpan`='$fengpan',`kaijiang`='$kaijiang' WHERE (`qishu`='".$rs['qishu']."')";
		echo $strSqls."<br>";
...
$curl->set_referrer("http://www.1680180.com/lottery/10016.html");
...
$html_data = $curl->fetch_url("http://www.1680180.com/Open/CurrentOpen?code=10016&_=0.3727684081279637");
$a=array('(',')');
$b=array('[',']');
