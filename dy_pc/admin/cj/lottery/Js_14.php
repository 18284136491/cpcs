...
if($_REQUEST['ac']=='re'){
	...
	$sql		= "select * from c_auto_14 where qishu=".$qi." order by id desc limit 1";
}else{
	$sql		= "select * from c_auto_14 where ok=0";
}
...
$hm[]		= $rs['ball_5'];
//根据期数读取未结算的注单
$sql1		= "select * from c_bet where type='新疆时时彩' and js=0 and qishu=".$qi." order by addtime asc";
...
$msql="update c_auto_14 set ok=1 where qishu=".$qi."";
$mysqli->query($msql) or die ("期数修改失败!!!");
}
if ($_GET['t']==1)    {
	echo "<script>window.location.href='../../Lottery/auto_14.php';</script>";
}
if($_REQUEST['ac']=='re'){
	echo "OK";
}
