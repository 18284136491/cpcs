...
?>
      <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#ffffff'" >
        ...
        <td><a href="hk_look.php?id=<?=$rows["id"]?>"><?=$rows["lsh"]?></a><br/><?=$time=='CN' ? get_bj_time($rows["adddate"]) : $rows["adddate"]?></td>
        ...
        <td><a href="<?=$_SERVER["REQUEST_URI"]?>&bank=<?=$rows["bank"]?>"><?=$rows["bank"]?>
        </a></td>
        <td><?=$rows["manner"]?><br/><?=$rows["address"]?></td>
        <td><?php
			  if($rows["status"]==1){
			  	$t_sum += $rows["money"];
			  	echo '<span style="color:#006600;">汇款成功</span>';
			  }else if($rows["status"]==2){
			  	$f_sum += $rows["money"];
			  	echo '<span style="color:#FF0000;">汇款失败</span>';
			  }else{
			  	$c_sum += $rows["money"];
