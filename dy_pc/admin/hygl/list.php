  ...
  <tr>
    ...
    <td height="24" align="center" nowrap bgcolor="#FFFFFF"><table width="655">
     <form name="form1" method="GET" action="list.php" >
      <tr>
        <td>内容：
          <input type="text" name="likevalue" value="<?=@$_GET['likevalue']?>"/>-<input type="text" name="likevalue2" value="<?php
          																								if ($_GET['likevalue2']=='') echo '0'; else echo @$_GET['likevalue2']?>"/>
            &nbsp;&nbsp;类型：
            ...
            <option value="pay_name" <?php if(@$_GET["selecttype"]=='pay_name') {?> selected <?php }?> >真实姓名</option>
             <option value="reg_ip" <?php if(@$_GET["selecttype"]=='reg_ip') {?> selected <?php }?>>注册IP</option>
              ...
              <option value="lmoney" <?php if(@$_GET["selecttype"]=='lmoney') {?> selected <?php }?>>额度</option>
      	...
      	?>
	        <tr align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>">
			  ...
			  <td>
				...
				?>
                <br><?=$rows["ag_zr_username"]?><br /><span style="color:#999999;"><?=$rows['www']?></span></td>
	          <td>
              	<?php
			  	if($rows["agents"]=='' || $rows["agents"]==null){
                ?>
