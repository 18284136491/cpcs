...
if(intval($web_site['d3'])==1)
{
	message('福彩3D系统维护，暂停下注！');
	exit();
}

$sql = "select id,class1,class2,class3,odds,modds,locked from lottery_odds where class1='3d' and class2='和值' order by ID asc";
...
<head>
	...
	function addzhbet(){
		...
		closeadds()
		<? if($stype==1){?>
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=1&bw="+bwhm+"&sw="+swhm+"&gw="+gwhm+"",
		<? }?>
		<? if($stype==2){?>
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=2&sw="+swhm+"",
		<? }?>
		<? if($stype==3){?>
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=3&bw="+bwhm+"&sw="+swhm+"",
		<? }?>
		<? if($stype==4){?>
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=4&bw="+bwhm+"&sw="+swhm+"&gw="+gwhm+"",
		<? }?>
		<? if($stype==5){?>
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=5&bw="+bwhm+"&sw="+swhm+"&gw="+gwhm+"",
		<? }?>
		<? if($stype==6){?>
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=6&bw="+bwhm+"",
		<? }?>
		{
			id:'opens',
			lock:true,
	...
	function hz_add(id){
		...
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=7&content="+id+"",
		{
			id:'opens',
			fixed: true,
			lock:true,
	...
	function dsdx_add(class3,id){
		...
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=8&class3="+class3+"&content="+id+"",
		{
			id:'opens',
			fixed: true,
			lock:true,
	...
	function kd_add(id){
		...
		art.dialog.open("buy_lottery/3d.php?uid=<?=$uid?>&stype=6&content="+id+"",
		{
			id:'opens',
			fixed: true,
			lock:true,
...
<div class="left_lottery_bg1"><? if($tcou>0){?>福彩3D第 <font color="#00FF00"><?=$trow['qihao']?></font> 期<? }else{?>期数未开盘<? }?></div>
<div class="left_lottery_bg2"><? if($tcou>0){?>北京时间：<?=$trow['fengpan']?><? }else{?>北京时间：暂停下注<? }?></div>
<div class="left_lottery_bg2"><? if($tcou>0){?>美东时间：<?=mdtime($trow['fengpan'])?><? }else{?>美东时间：暂停下注<? }?></div>
<div class="left_lottery_bg3"><? if($tcou>0){?>
<DIV id=time_conter style="DISPLAY: none;">距离封盘：<SPAN id=h></SPAN>&nbsp;小时&nbsp;<SPAN id=m></SPAN>&nbsp;分钟&nbsp;<SPAN id=s></SPAN>&nbsp;秒</DIV>
<SCRIPT language=javascript> 
function n2(n)
{
...
<?
$sqlk = "select * from lottery_k_3d where ok=1 order by qihao desc,id desc limit 10";
