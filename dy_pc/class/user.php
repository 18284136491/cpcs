...
class user{
    static function login($username,$passwrod){ //登陆，进行验证，和信息更新,产生UID
		global $mysqli;
	   	$sql	=	"select uid,is_daili,gid,is_delete,is_stop,reg_date,password,username from k_user where username=? and password=? limit 1";
		$stmt = $mysqli->prepare($sql);
		$stmt->bind_param("ss",$username,md5($passwrod));
		$stmt->execute();
		$stmt->bind_result($t_uid,$t_is_daili,$t_gid,$t_is_delete,$t_is_stop,$t_reg_date,$t_password,$t_username);
		$stmt->fetch();
		$stmt->close();
