...
<body>
    <div class="kj_jl">
        ...
        <table cellspacing="0" cellpadding="0" border="0" class="tb_list">
            <tr class="tit">
                <td width="100">期数</td>
                <td width="102">开奖时间</td>
                <td width="368" colspan="8">开出号码</td>
                <td colspan="4">总和</td>
                <td>龙虎</td>
            </tr>
            <?php
                ...
                if($id) {
                    ...
                    while($row = $query->fetch_array()) {
                        ...
                        <tr class="list">
                            ...
                            <td class="gdsf"><em class="n_<?=$row['ball_1']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_2']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_3']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_4']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_5']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_6']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_7']?>"></em></td>
                            <td class="gdsf"><em class="n_<?=$row['ball_8']?>"></em></td>
                            <td width="46"><?=Ssc_Auto($hm,1)?></td>
                            <td width="46"<?=Ssc_Auto($hm,2) == '大' ? ' class="red"' : ''?>><?=Ssc_Auto($hm,2)?></td>
                            <td width="46"<?=Ssc_Auto($hm,3) == '双' ? ' class="red"' : ''?>><?=Ssc_Auto($hm,3)?></td>
                            <td width="46"<?=Ssc_Auto($hm,4) == '尾大' ? ' class="red"' : ''?>><?=Ssc_Auto($hm,4)?></td>
                            <td width="46"<?=Ssc_Auto($hm,5) == '龙' ? ' class="red"' : ''?>><?=Ssc_Auto($hm,5)?></td>
                        </tr>
                        <?php
                    }
                }
            ?>
            <tr>
                <td colspan="15"><?php echo $pageStr; ?></td>
            </tr>
        </table>
    </div>
