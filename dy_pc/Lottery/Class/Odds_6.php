...
//开始读取赔率
$sql		= "select * from c_odds_6 order by id asc";
...
//开始读取期数
$sql		= "select * from c_opentime_6 where kaipan<='".date("H:i:s",$lottery_time)."' and kaijiang>='".date("H:i:s",$lottery_time)."' order by id asc";
$query		= $mysqli->query($sql);
$qs		= $query->fetch_array();
if($qs){
	$qishu		= substr(date("Ymd",$lottery_time), 2,6).BuLings($qs['qishu']);
