...
//用户输赢
$sql = "select round(SUM(money), 2) as yk from c_bet where type='幸运飞艇' and uid='$uid'";
$query = $mysqli->query($sql);
$rs = $query->fetch_assoc();
$z_money = $rs['yk'];
$sql = "select round(SUM(win), 2) as yk from c_bet where type='幸运飞艇' and uid='$uid' and win >= 0";
$query = $mysqli->query($sql);
$rs = $query->fetch_assoc();
$z_win = $rs['yk'];
...
//十期开奖结果
$sql = "select qishu,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8,ball_9,ball_10 from c_auto_8 where ok=1 order by id desc limit 10";
$query = $mysqli->query($sql);
$kj_list = array();
