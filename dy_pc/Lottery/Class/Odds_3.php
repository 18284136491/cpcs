...
//开始读取赔率
$sql		= "select * from c_odds_3 order by id asc";
...
//开始读取期数
$sql		= "select * from c_opentime_3 where kaipan<='".date("H:i:s",$lottery_time)."' and kaijiang>='".date("H:i:s",$lottery_time)."' order by id asc";
$query		= $mysqli->query($sql);
...
if($qs){
	...
	$sql		= "select * from c_opentime_3 where qishu=1";
	$query		= $mysqli->query($sql);
	$qs		= $query->fetch_array();
	if($qs){
