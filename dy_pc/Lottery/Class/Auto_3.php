...
//用户输赢
$sql = "select round(SUM(money), 2) as yk from c_bet where type='广东快乐10分' and uid='$uid'";
$query = $mysqli->query($sql);
$rs = $query->fetch_assoc();
$z_money = $rs['yk'];
$sql = "select round(SUM(win), 2) as yk from c_bet where type='广东快乐10分' and uid='$uid' and win >= 0";
$query = $mysqli->query($sql);
$rs = $query->fetch_assoc();
$z_win = $rs['yk'];
...
$endDate = $l_date . ' 23:59';
$date = "datetime > '$startDate' and datetime < '$endDate'";
$sql = "select ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8 from c_auto_3 where $date and ok=1 order by id asc";
$query = $mysqli->query($sql);
$history = array();
...
//十期开奖结果
$sql = "select qishu,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8 from c_auto_3 where ok=1 order by id desc limit 10";
$query = $mysqli->query($sql);
$kj_list = array();
